...
namespace MyNamespace
{
    ...
    public Class1() 
    {
        ...
        public void CheckDocumentOrFolderExistence(ScriptParameters sp)
        {
            ...
            try
            {
                ...
                string objectId = sp.InputVariables["OBJECTID"].ToString();  // Expects Folder.InstanceId in Input variables
                ...
                try
                {
                    Agility.Sdk.Model.Capture.Folder folder = captureDocumentService.GetFolder(sessionId, null, objectId);
                    objectFound = true;
                    log.AppendLog("Object Id found as folder");         
                }
                ...
                if (objectFound == false)
                {
                    try
                    {
                        Agility.Sdk.Model.Capture.Document document = captureDocumentService.GetDocument(sessionId, null, objectId);
                        objectFound = true;
                        log.AppendLog("Object Id found as document");           
                    }
                    catch (System.Exception)
                    {
                ...
                sp.OutputVariables["ObjectFound"] = objectFound;
